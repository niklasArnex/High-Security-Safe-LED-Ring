...
uint8_t encoder_pinSW = 1;


volatile int32_t counter = 0;
volatile uint8_t prevState = 0;  // Store previous state (00, 01, 10, 11)

volatile int32_t stepCounter = 0;  // Additional step counter
int stepsPerUpdate = 2;  // Number of steps before updating the main counter
...
/// @brief get the current state of the encoder's counter.
...
void isr_re()
{
    ...
    // Determine rotation direction based on the lookup table
    RotationDirection_t direction = lookupTable[prevState][state];

    // Increment or decrement step counter based on rotation direction
    ...
    // Only update the main counter when the step counter reaches the threshold
    if (abs(stepCounter) >= stepsPerUpdate) {
        if (stepCounter > 0) {
            counter--;
        } else if (stepCounter < 0) {
            counter++;
        }
        stepCounter = 0;  // Reset step counter after updating
    }
...
void SWButtonPress(uint32_t &btn_state, int &btn_counter) {
    if (digitalRead(encoder_pinSW) == 0) {
        if (btn_state == BTN_NOT_PRESSED) {
